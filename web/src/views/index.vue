    ...
    <main class="space-stage" :aria-label="$t('home.experienceAria')">
      <div class="space-camera" aria-hidden="true">
        <div
          ...
          :style="{ backgroundImage: `linear-gradient(90deg,rgba(1,7,20,.94),rgba(1,7,20,.26) 58%,rgba(1,7,20,.5)),url(${scene.image})` }"
        />
...
const timelineStyle = computed(() => ({ '--t': current.value, '--node': activeNode.value, '--fraction': current.value - Math.floor(current.value) }))
...
const isInteractiveTarget = (targetElement: EventTarget | null) =>
  targetElement instanceof HTMLElement && Boolean(targetElement.closest('button,a,input,select,textarea,[role="button"]'))
...
<style scoped>
.timeline{--t:0;position:fixed;inset:0;width:100vw;height:100svh;overflow:hidden;background:#020817;color:#f5f9ff;font-family:var(--aix-font);touch-action:none;user-select:none}.timeline :deep(.app-header){z-index:12000}.space-stage{position:absolute;inset:0;overflow:hidden;perspective:1200px}.space-camera{pointer-events:none}.space-camera{position:absolute;inset:-10%;transform:perspective(900px) translateZ(calc((var(--fraction) - .5) * 230px)) rotate(calc(sin(var(--t) * 3.14159) * 10deg)) scale(calc(1.06 + sin(var(--t) * 3.14159) * .15));will-change:transform}.scene-image{position:absolute;inset:0;background-position:center;background-size:cover;opacity:clamp(0,1 - abs(var(--t) - 0),1);transform:scale(calc(1.06 + abs(var(--t) - 0) * .1))}.scene-image--1{opacity:clamp(0,1 - abs(var(--t) - 1),1);transform:scale(calc(1.06 + abs(var(--t) - 1) * .1)) rotate(calc((var(--t) - 1) * -6deg))}.scene-image--2{opacity:clamp(0,1 - abs(var(--t) - 2),1);transform:scale(calc(1.06 + abs(var(--t) - 2) * .1)) rotate(calc((var(--t) - 2) * 8deg))}.scene-image--3{opacity:clamp(0,1 - abs(var(--t) - 3),1);transform:scale(calc(1.06 + abs(var(--t) - 3) * .1)) rotate(calc((var(--t) - 3) * -7deg))}.scene-image--4{opacity:clamp(0,1 - abs(var(--t) - 4),1);transform:scale(calc(1.06 + abs(var(--t) - 4) * .08))}.speed-lines{position:absolute;inset:-20%;opacity:calc(sin(var(--t) * 3.14159) * .46);background:repeating-conic-gradient(from 0deg at 50% 50%,transparent 0 3deg,rgba(69,170,255,.13) 3.2deg 3.4deg);transform:scale(1.4)}.vignette{position:absolute;inset:0;background:radial-gradient(circle,transparent 30%,rgba(0,4,16,.78))}
.opening{position:fixed;z-index:10000;inset:0;display:grid;place-items:center;overflow:hidden;background:#020817;pointer-events:none;transition:opacity .85s ease,visibility .85s}.opening-space,.opening-tunnel,.opening-energy,.opening-vignette{position:absolute;inset:0}.opening-space{overflow:hidden;animation:opening-flight 2.65s cubic-bezier(.18,.7,.22,1) both}.opening-tunnel{inset:-8%;background:url('/assets/timeline-00-time-tunnel.png') center/cover no-repeat;filter:saturate(1.12) contrast(1.08);animation:tunnel-roll 2.65s cubic-bezier(.18,.7,.22,1) both;will-change:transform,filter}.opening-energy{inset:-25%;background:repeating-conic-gradient(from 12deg at 50% 50%,transparent 0 5.5deg,rgba(61,174,255,.13) 5.7deg 5.95deg,transparent 6.1deg 12deg);mix-blend-mode:screen;animation:energy-spin 2.65s cubic-bezier(.18,.7,.22,1) both}.opening-vignette{background:radial-gradient(circle at center,transparent 12%,rgba(1,7,20,.12) 39%,rgba(1,7,20,.84) 100%),linear-gradient(180deg,rgba(1,7,20,.36),transparent 34%,rgba(1,7,20,.5))}.opening-copy{position:relative;z-index:2;display:flex;align-items:center;flex-direction:column;gap:18px;padding:28px;text-align:center;animation:opening-copy 2.3s ease both}.timeline:not(.intro-complete) :deep(.app-header){opacity:0;visibility:hidden}.timeline :deep(.app-header){transition:opacity .45s ease,visibility .45s}.opening p,.opening h1{margin:0;color:#f7fbff;font-weight:700;letter-spacing:.1em;text-shadow:0 4px 18px #020817,0 12px 42px #000}.opening p{font-size:clamp(18px,2vw,28px)}.opening h1{font-size:clamp(48px,8vw,112px);letter-spacing:.04em}.opening-copy span{width:clamp(70px,9vw,140px);height:2px;background:#168bff;box-shadow:0 0 24px #168bff;animation:scan 1.25s .18s ease-in-out both}.intro-complete .opening{opacity:0;visibility:hidden;pointer-events:none}.intro-complete .opening-tunnel{transform:scale(1.68)}
