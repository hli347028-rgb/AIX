    ...
    <div class="market-shell">
      ...
      <div class="market-stats" :aria-label="$t('market.stats24h')">
        <div class="stat">
          ...
          <strong>{{ formatPrice(dayLow) }}</strong>
        </div>
        <div class="stat">
          <span>{{ $t('market.high24h') }}</span>
          <strong>{{ formatPrice(dayHigh) }}</strong>
        </div>
      </div>
...
.market-stats>.stat strong{
  font-size:20px;
  font-variant-numeric:tabular-nums;
  letter-spacing:-.03em;
}
@media(max-width:759px){
  .market-stats>.stat{padding:14px 8px;gap:6px}
  .market-stats>.stat span{font-size:9px}
  .market-stats>.stat strong{font-size:16px}
}
</style>
