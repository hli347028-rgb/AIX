...
export default {
    ...
    methods: {
        ...
        config_update(id) {
            ...
            let hint = '静态利率填百分数如 0.5；直推/W 收益系数填小数如 0.2 表示 20%；W 晋级金额按小区业绩填写且必须逐级递增；出局倍数默认 4；USDT/WIN 充值最小值须 ≥ 10；提现审核阈值填 0 表示不审核；AIX兑换审核阈值填百分数如 40，表示全网当日兑换超过今日AIX的40%后后续需审核（默认100）；交易所划转限额须满足：单笔下限 ≤ 单笔上限 ≤ 单日上限'
